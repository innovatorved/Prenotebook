...
router.post(
  ...
  [
    ...
    body(
      "description",
      "Minimum 10 characters neccessary in Description",
    ).isLength({ min: 10 }),
  ],
  async (req, res) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ success, error: "Internal Server try after some time2" });
      // console.error(error.message);
    }
  },
);
...
router.put("/updatenote/:id", fetchUserDetails, async (req, res) => {
  try {
    ...
    if (NoteUser !== ReqFromUser) {
      return res.status(401).send("UnAuthorished Access");
    } // Check the owner of note is real
...
router.get("/sharedNote/:id", async (req, res) => {
  try {
    ...
    const thisNote = await Notes.findById(noteId, async (err, note) => {
      if (err || note.share === false) {
        return res.json({ success: "false", error: "Note Not Found" });
      }
      const mynote = await note.populate({
        path: "user",
        model: "user",
        select: { _id: 0, name: 1, username: 1 },
      });
